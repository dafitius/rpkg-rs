...
use super::resource_container::{ResourceContainer, ResourceContainerError};
...
pub enum PackageManagerError {
    ...
    #[error("Cannot find packagedefinition file: {0}")]
    MissingPackageDefinitionFile(#[from] std::io::Error),

    #[error("Couldn't mount container: {0}")]
    MountingError(#[from] ResourceContainerError),
}
...
impl PackageManager {
    ...
    fn mount_resource_packages_in_partition(resource_container: &mut ResourceContainer, partition_info: &PartitionInfo, runtime_path: &PathBuf) -> Result<(), PackageManagerError> {
        resource_container.mount_partition(partition_info, runtime_path).map_err(PackageManagerError::MountingError)
    }

    fn get_partition_infos(path: &Path) -> Result<Vec<PartitionInfo>, PackageManagerError> {
        let bytes = utils::get_file_as_byte_vec(path).map_err(PackageManagerError::MissingPackageDefinitionFile)?;
        let deciphered_data = match Xtea::is_encrypted_text_file(&bytes) {
            ...
            false => match String::from_utf8(bytes) {
                Ok(v) => v,
                Err(e) => return Err(PackageManagerError::TextEncodingError(e)),
            },
        };
